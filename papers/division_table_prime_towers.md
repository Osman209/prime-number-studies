...
We start from the elementary table of divisions $n / d$ with $n$ and $d$ odd, and show that a change of coordinates turns it into a completely transparent object. Writing $n = 2H + 3$ and $d = 2j + 1$, the remainder field becomes $R(H,j) = 2(H - j + 1) mod (2j+1)$, and the exact cells of each column form a single arithmetic progression. Non-trivial exact divisions are then parametrized by the bivariate map $\Phi(j,t) = 2jt + j + t - 1$, whose image is exactly the set of row indices of odd composites — a statement that turns out to be the sieve of Sundaram in a shift of coordinates. We prove the factor symmetry $\Phi(j,t) = \Phi(t,j)$, identify the triangular fundamental domain $j \le t$ as trial division up to $\sqrt n$, and show that the number of ordered preimages of $H$ is $\tau(n) - 2$, so that the divisor function appears as the collision count of a family of straight lines.

The table is then decomposed into divisibility layers. Every composite layer is an intersection of prime-power layers, so the prime-power columns form a generating system and every other column is redundant. Depth in the resulting nested towers is the $p$-adic valuation, and the valuation vector $V(n)$ reconstructs $n$ together with $\tau$, $\varphi$, $\mu$ and $\Lambda$. We quantify the resulting compression and show that it is exactly of prime-number-theorem size, growing like $(\log N)/2$. Every claim is verified numerically with no failures. The exposition stops deliberately before any use of the Riemann zeta function.
...
Take the odd integers $3, 5, 7, 9, 11, \ldots$ and use them twice: once as **row labels** (the numerator $n$) and once as **column labels** (the denominator $d$). In the cell at row $n$ and column $d$, write the result of the division:
...
**(ii) A column is periodic.** Look down the column $\div 3$: the exact cells are at $n = 9, 15, 21, 27, 45, \ldots$ — every third odd number. Column $\div 5$ is exact at $n = 15, 25, 35, 45, \ldots$ — every fifth odd number. Every column is a perfectly regular comb; the columns differ only in their period and in where the comb starts.

**(iii) The table is mostly redundant.** Column $\div 9$ is exact only at rows where column $\div 3$ is already exact ($9, 27, 45, \ldots$) — it is a *sub-comb* of $\div 3$. Similarly $\div 25 \subset \div 5$. And a column like $\div 15$ is exact exactly where $\div 3$ **and** $\div 5$ are both exact. So a composite column tells you nothing you could not read off the prime-power columns.
...
The table as written is awkward because the row and column labels are the odd numbers $3, 5, 7, \ldots$, and "every third odd number" is a clumsy thing to say. So we re-label by *position*:
...
In these coordinates the three observations become clean statements: a column is an arithmetic progression in $H$ (§4), the composite rows are the image of a single bilinear map (§5), and the layers nest (§9). That is the whole content of the coordinate change — it converts "every third odd number starting at 9" into "$H \equiv 0 mod 3$".

**Figure 1** shows the four resulting pictures: the remainder field with the exact cells marked, the columns as arithmetic progressions, the map $\Phi$ as a family of straight lines, and the nested towers.
...
**Proposition 1 (unified remainder formula).** For all $H \ge 0$ and $j \ge 1$,
...
*Proof.* $n - d = (2H + 3) - (2j + 1) = 2(H - j + 1) = 2k$, so $n = d + 2k$ and $n \equiv 2k (mod d)$. ∎

The residue convention is not cosmetic: when $j \gt H + 1$ the denominator exceeds the numerator and $k$ is negative, and the formula continues to hold only with that reading. For instance $n = 9$, $d = 11$ gives $k = -1$ and $2k mod 11 = 9 = 9 mod 11$.

*Verified:* 419,400 cells ($H \lt 600$, $j \lt 700$), including the whole $k \lt 0$ region — zero mismatches.
...
3. $d | k$;
4. $H \equiv j - 1 (mod d)$;
5. $H \equiv(d - 3)/2 (mod d)$.

*Proof.* (1) ⟺ (2) is the definition. By Proposition 1, $R = 0$ ⟺ $d | 2k$; since $d$ is odd, $\gcd(2,d) = 1$, so this is $d | k$, giving (3). Substituting $k = H - j + 1$ gives $H \equiv j - 1 (mod d)$, which is (4); and $j - 1 = (d-1)/2 - 1 = (d-3)/2$ gives (5). ∎

*Verified:* all five conditions agree on every cell with $H \lt 3000$, $j \lt 80$ — zero mismatches.
...
equivalently $n = d\cdot(2t + 1)$. The case $t = 0$ is the trivial cell $n = d$; non-trivial exact division means $t \ge 1$.
...
## 5. The factor map $\Phi$

Non-trivial exactness means $n = d\cdot q$ with both factors odd and greater than one. Write $d = 2j+1$ and $q = 2t+1$ with $j, t \ge 1$. Then
...
**Definition.** $\Phi(j,t) = 2jt + j + t - 1$, for $j, t \ge 1$.

**Theorem 1 (characterization of odd composites).** For odd $n = 2H + 3 \ge 9$:
...
*Proof.* If $H = \Phi(j,t)$ then $n = (2j+1)(2t+1)$ with both factors $\ge 3$, so $n$ is composite. Conversely, if $n$ is an odd composite, write $n = d\cdot q$ with $d, q \ge 3$; both are odd, so $d = 2j+1$, $q = 2t+1$ with $j, t \ge 1$, and the computation above gives $H = \Phi(j,t)$. ∎

*Verified:* odd $n \le 200001$, zero mismatches.
...
so Theorem 1 reads: *$2m + 1$ is composite if and only if $m = j + t + 2jt$ for some $j, t \ge 1$*. This is the **sieve of Sundaram**, published in 1934. The identity $\Phi(j,t) + 1 = j + t + 2jt$ is verified identically.
...
**Proposition 3 (factor symmetry).** $\Phi(j,t) = \Phi(t,j)$.

*Proof.* $2jt + j + t - 1$ is symmetric in $j$ and $t$. ∎

The symmetry reflects the exchange $d ↔ q$ of the two factors, so restricting to the triangle $1 \le j \le t$ counts each unordered factor pair once. This triangle is not an arbitrary convenience:
...
*Proof.* The first two equivalences are immediate. For the last, $d \le q \Leftrightarrow d^{2} \le dq = n \Leftrightarrow d \le \sqrt n$. ∎

**Proposition 5 (the diagonal is the odd squares).** $\Phi(j,j) = 2j^{2} + 2j - 1$, and $2\cdot \Phi(j,j) + 3 = (2j+1)^{2}$.

*Proof.* Direct substitution. ∎

*Verified:* symmetry, the triangle equivalence and the diagonal identity all hold with zero violations over $j, t \lt 300$.

So the classical rule "test divisors up to $\sqrt n$" is, in this picture, exactly the statement that the fundamental domain of the symmetry $\Phi(j,t) = \Phi(t,j)$ is a triangle.

---

## 7. Multiplicity and the divisor function

**Theorem 2 (number of preimages).** For odd $n = 2H + 3$, the number of **ordered** pairs $(j,t)$ with $j,t \ge 1$ and $\Phi(j,t) = H$ is

```
#Φ⁻¹(H)  =  τ(n) − 2 ,
```

where $\tau$ is the divisor-counting function. The number of **unordered** pairs is $\lceil(\tau(n) - 2)/2\rceil$.

*Proof.* Ordered pairs $(j,t)$ correspond bijectively to ordered factorizations $n = d\cdot q$ with $d, q \ge 3$, i.e. to divisors $d$ of $n$ with $1 \lt d \lt n$. Since $n$ is odd, all its divisors are odd, so none is excluded by parity. There are $\tau(n) - 2$ such divisors. For the unordered count, pairs come in swapped couples except the fixed point $d = q = \sqrt n$, which occurs iff $n$ is a square; in that case $\tau(n) - 2$ is odd and the ceiling accounts for it. ∎

*Verified:* odd $n \le 40005$, zero mismatches.
...
Theorem 2 is consistent with Theorem 1 at the boundary: $n$ is prime iff $\tau(n) = 2$ iff the preimage count is $0$ iff $H \notin \mathrm{Im}(\Phi)$.

---

## 8. Line geometry in the $(j, H)$-plane

For fixed $t$, $\Phi$ is affine in $j$:
...
A **collision** of several lines at the same height $H$ means several factorizations of the same integer, and by Theorem 2 the number of lines through height $H$ is $\tau(n) - 2$. Panel (C) of Figure 1 shows this directly: the row indices of primes are precisely the heights that no line reaches.

---

## 9. Layers and prime-power towers

For odd $d \ge 3$ define the **divisibility layer**
...
**Theorem 3 (a composite column is an intersection).** If $d = \prod_p p^{a_p}$ then
...
**Proposition 6 (towers nest).** $Z_p \supseteq Z_{p^{2}} \supseteq Z_{p^{3}} \supseteq \cdots$.

*Proof.* $p^{a+1} | n$ implies $p^a | n$. ∎

*Verified:* every odd $d \le 2000$ against $H \lt 4000$, zero violations.

**Consequence — the redundancy of the table is exact.** Composite columns are not independent objects: $Z_{15} = Z_3 \cap Z_5$, $Z_{75} = Z_3 \cap Z_{25}$, and so on. The prime-power columns form a generating system under intersection, and nothing else in the table is a generator.
...
Define $v_p(n) = \max { a \ge 0 : p^a | n }$ — equivalently, *how deep row $H$ sits in the tower over $p$* — and collect these depths:

```
V(n) = ( v_3(n), v_5(n), v_7(n), … ) .
```

**Theorem 4 (completeness).** For odd $n$, the vector $V(n)$ determines $n$, via $n = \prod_p p^{v_p(n)}$, and only finitely many entries are nonzero.

*Proof.* This is the uniqueness half of the fundamental theorem of arithmetic; finiteness holds because $p^{v_p(n)} \le n$. ∎
...
In tower language, $\Lambda$ is nonzero exactly when **one and only one tower is active**, at any depth. This is the sharpest thing the picture says: $\Lambda$ sees *which* towers are lit, not how brightly.

*Verified:* $\tau$, $\varphi$, $\mu$ rebuilt from $V(n)$ for all odd $n \le 200001$ — zero violations.
...
| test | range | failures |
|---|---|---|
| remainder formula, including $j \gt H+1$ | 419,400 cells | 0 |
| equivalence of the five exact-cell conditions | $H \lt 3000$, $j \lt 80$ | 0 |
| composite coverage by $\mathrm{Im}(\Phi)$ (Thm 1) | odd $n \le 200001$ | 0 |
| $\Phi(j,t) + 1 = j + t + 2jt$ (§5.1) | $j,t \lt 200$ | 0 |
| preimage multiplicity $\tau(n) - 2$ (Thm 2) | odd $n \le 40005$ | 0 |
| symmetry, diagonal, $j \le t \Leftrightarrow d \le \sqrt n$ | $j,t \lt 300$ | 0 |
| $Z_d = ⋂ Z_{p^a}$ (Thm 3) | odd $d \le 2000$, $H \lt 4000$ | 0 |
| $\tau, \varphi, \mu$ from $V(n)$ | odd $n \le 200001$ | 0 |
...
**Over the range $\sqrt N$, which is what primality testing actually uses.** Deciding primality of every $n \le N$ requires only denominators $d \le \sqrt N = 1000$. There are $499$ odd columns in that range and $167$ odd prime layers:
...
**A comparison to avoid.** Dividing the column count over one range by the layer count over another — for example $499999 / 167 \approx 2994$ — inflates the figure by roughly $\sqrt N$ and describes a method nobody would use, since trial division never consults a divisor beyond $\sqrt n$. Compression figures here must fix a single range on both sides.

**Both honest factors are prime-number-theorem sized.** The first is the reciprocal density of primes among the odd numbers up to $\sqrt N$. The second satisfies
...
- The remainder field of the odd division table is $R(H,j) = 2(H-j+1) mod (2j+1)$, valid on the whole table including $d \gt n$.
- Every column's exact cells form one arithmetic progression, of period $d$ starting at $(d-3)/2$.
- $\mathrm{Im}(\Phi)$ is exactly the set of row indices of odd composites (Theorem 1).
- Ordered preimage multiplicity is $\tau(n) - 2$ (Theorem 2), so the divisor function is the collision count of the line family.
- The symmetry $\Phi(j,t) = \Phi(t,j)$ has the triangle $j \le t$ as fundamental domain, and that triangle *is* trial division to $\sqrt n$; its diagonal is the odd squares.
...
- The valuation vector reconstructs $n$, $\tau$, $\varphi$, $\mu$, $\Lambda$.
...
- **No faster primality test.** The fundamental domain reproduces trial division to $\sqrt n$, and §12.1 shows the saving over it is a factor $\approx 3$, asymptotically only $(\log N)/2$.
...
1. V. Ramaswami Aiyar, *Sundaram's Sieve for Prime Numbers*, The Mathematics Student **2** (1934), 73. *(The map $\Phi$ of §5.)*
2. G. H. Hardy and E. M. Wright, *An Introduction to the Theory of Numbers*, 6th ed., Oxford University Press, 2008. *(Fundamental theorem of arithmetic; $\tau$, $\varphi$, $\mu$, $\Lambda$; §§9–11.)*
