...
We begin with the sieve cycle of $W = 2\cdot 3\cdot 5\cdot 7\cdot 11\cdot 13 = 30030$, represented by its reduced residues modulo $W$, and count the pairs separated by an even distance $H$. We prove that this count is given exactly by a local product with no free parameters,

```
W(H) = ∏_{p | W, p | H} (p − 1) · ∏_{p | W, p ∤ H} (p − 2) ,
```

and that after normalization it is the Hardy–Littlewood singular series truncated at $13$, up to one explicit constant. We then study how this exact local count relates to the observed frequency of gaps between *consecutive* primes, measuring gap distributions in windows of width $4\times 10^{7}$ at heights $10^{7}, 10^{9}, 10^{11}, 10^{13}$.

The empirical part is reported with its uncertainty made explicit. A stretched exponent $\beta \gt 1$ fits the data noticeably better than $\beta = 1$ at every height, and it transfers out of sample. It is nevertheless **not** an exponent: it moves about seven times more when the fitting window in $H$ is changed than when the height is changed by six orders of magnitude, and even the *sign* of its drift in height depends on the window. What it absorbs is identified: the theoretical envelope constant $\lambda = 1/\log X$ is not exact, the measured $\lambda \cdot \log X$ lying between $1.09$ and $1.16$. The recommended model is therefore $A(H) \propto S(H)\cdot \exp(-\lambda H)$ with a single calibrated $\lambda$ and no shape exponent — not because the shapeless model wins a prediction contest (it does not), but because $\beta$ fails the stability test that any genuine exponent must pass.
...
*Proof.* The condition is local at each prime $p | W$: we need $a \not\equiv 0$ and $a \not\equiv -H (mod p)$. If $p | H$ the two forbidden residues coincide, leaving $p - 1$ admissible choices; if $p \nmid H$ they are distinct, leaving $p - 2$. The primes dividing $W$ are distinct, so by the Chinese remainder theorem the local choices combine multiplicatively. ∎

**Corollary 1.** If $H$ is odd then the factor at $p = 2$ is $2 - 2 = 0$, so $W(H) = 0$.

**Corollary 2.** $W(H)$ depends on $H$ only through which primes $p \le 13$ divide it, hence it is periodic in $H$ with period $W$.
...
one has $S(H) = \kappa \cdot \mathfrak{S}(H)$ with $\kappa = \prod_{p \gt 13} (1 - 1/(p-1)^{2})^{-1}$ **whenever every odd prime factor of $H$ is at most 13**, and $S(H) \ne \kappa \cdot \mathfrak{S}(H)$ otherwise.

*Verified numerically:* $2C_{2} = 1.320323652$ (literature value $1.3203236$), and the ratio $S(H)/\mathfrak{S}(H)$ equals $1.01802$ for $H = 2, 6, 30, 42, 210, 2310$, while it is $0.95439$ at $H = 34 = 2\cdot 17$ and $0.99589$ at $H = 94 = 2\cdot 47$ — exactly the cases with an odd prime factor above $13$.

So the truncation is not free: it introduces a fixed multiplicative constant $\kappa$ (absorbed by any overall normalization, hence harmless) plus a genuine error at those $H$ whose odd part has a large prime factor. Below $H = 100$ the affected values are few and the error is at most about $5$%, which is why extending $W$ changes little at these ranges.
...
**The data.** All measurements below come from segmented sieves over windows of width $\Delta X = 4\times 10^{7}$ placed at $X = 10^{7}, 10^{9}, 10^{11}, 10^{13}$:
...
**Fitting protocol, stated because the numbers depend on it.** Even $H$ in $[2, H_{\max} ]$ with at least $20$ occurrences; least squares on $\log A(H)$; $R^{2}$ reported in log space. Value-space fitting inflates $R^{2}$ substantially because it is dominated by the few smallest gaps, and should not be used.

---

## 7. Does the data support a shape exponent $\beta \gt 1$?

Replacing the envelope $\exp(-\lambda H)$ by the stretched form $\exp(-\lambda H^{\beta})$ gives a visibly better fit at every height. The question is whether $\beta$ is an exponent or an absorber. Three tests decide it.

### 7.1 Fit quality (the weakest evidence)

At $H_{\max} = 120$, log-space $R^{2}$:

| X | $\beta = 1, \lambda = 1/\log X$ | $\lambda$ free | $\lambda, \beta$ free | fitted $\beta$ |
|---|---|---|---|---|
...
Adding $\beta$ buys about $0.0005$ in $R^{2}$ over a free $\lambda$. High $R^{2}$ is not evidence: this column mainly shows that a two-parameter model beats a one-parameter model, which is guaranteed.
...
| X \ $H_{\max}$ | 60 | 80 | 120 | 160 | 200 | 240 |
|---|---|---|---|---|---|---|
...
Changing the window at a fixed height moves $\beta$ by up to $0.065$. Changing the height by six orders of magnitude at a fixed window moves it by:

| $H_{\max}$ | $\beta(10^{7}) \to \beta(10^{13})$ | total drift |
|---|---|---|
| 80 | 1.1195 → 1.0920 | −0.0275 |
| 120 | 1.0750 → 1.0655 | −0.0095 |
| 200 | 1.0580 → 1.0625 | **+0.0045** |

So the window dependence is roughly seven times the height dependence, and at $H_{\max} = 200$ the height drift even changes sign. A statement such as "$\beta$ drifts toward $1$ as $X$ grows" is therefore not supported: it is an artifact of a particular window. Panel (B) of Figure 1 shows this directly.

### 7.3 What $\beta$ is absorbing

Fitting $\lambda$ with $\beta = 1$ and comparing against the theoretical value:

| X | log X | fitted $\lambda$ | $1/\log X$ | $\lambda \cdot \log X$ |
|---|---|---|---|---|
...
The envelope constant in (6.1) is wrong by $9-16$%, and drifts. That mis-specification is a real, systematic feature of the model, and a free shape exponent is well placed to soak it up. The correct response is to calibrate $\lambda$ rather than to introduce $\beta$.

---

## 8. Out-of-sample transfer

The strongest available test is to calibrate at one height and predict at another with no refitting of shape. Training at $10^{7}$, predicting at $10^{13}$:

| $H_{\max}$ | $\beta =1, \lambda =1/\log X$ (0 transported) | $\beta =1, \lambda \cdot \log X$ transported (1) | $\lambda, \beta$ transported (2) |
|---|---|---|---|
| 60 | 3.6% | — | 3.5% |
| 80 | 5.4% | 4.3% | 3.3% |
| 120 | 10.0% | 5.9% | 2.5% |
...
(mean relative error; log-space $R^{2}$ for the same runs is $0.986-0.995$ for the first column and $0.996-0.9995$ for the last.)
...
with **no shape exponent**. The justification is §7.2, not §8: $\beta$ is rejected because it is not stable under a change of fitting window and its height drift is not even sign-definite, which is what disqualifies a quantity from being an exponent. It is not rejected for predicting badly — it predicts well, and that should be stated plainly.

The residual $9-16$% discrepancy in $\lambda \cdot \log X$ is left explicit rather than absorbed. It is a genuine deficiency of the Cramér envelope at these heights and is the honest place for future work to attach.
...
- **High $R^{2}$ is not evidence.** §7.1 shows a $0.0005$ gain being obtainable from an unstable parameter, and value-space fitting inflates the figure further.
- The height results come from single windows of width $4\times 10^{7}$. They carry sampling error that is not quantified here; repeated windows at each height would be needed for genuine confidence intervals.
- The rejection of $\beta$ rests on a stability argument (§7.2), not a prediction argument. §8 goes the other way and is reported as such.
...
4. Segment-sieve a window of width $\Delta X$ at each height and histogram the consecutive-prime gaps.
...
`verify_paper3.py` produces §§4–8; `robust_paper3.py` produces the window sweeps of §7.2 and §8; `fig3.py` produces Figure 1. Environment: Python 3, NumPy 2.4, SymPy 1.14. Gap data: segmented sieve, $\Delta X = 4\times 10^{7}$ per height.
